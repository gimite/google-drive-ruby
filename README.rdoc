...
Example to read/write file in Google Drive with token that was stored in a local file (just request access authorization once)
  CREDENTIALS_PATH = File.join(Dir.pwd, 'stored_token.json')
  session = GoogleDrive.saved_session(CREDENTIALS_PATH, nil, CLIENT_ID, CLIENT_SECRET)
  session.files.each do |file|
    p file.title
  end
  
Example to authorize by Refesh Token (offline mode - https://developers.google.com/drive/web/auth/web-server), used in Web Server that cannot log in Google Account by a web browser

  REFRESH_TOKEN = "YOUR REFRESH TOKEN" # Can be get from file stored_token.json above
  client = Google::APIClient.new
  auth = client.authorization
...
Ruby 1.9.x or later. Checked with Ruby 1.9.3 and 2.1.3 and 2.2.2.
